...
{\bf Singular valuer decomposition distance.}
...
however, we note that the MDS distnace is defined with a population of ``All'' mobile and non-mobile scholars, whereas the predictions made in Table~\ref{supp:table:rmse_power_table} use the raw frequency, which likely confounds this result. Also, running time of MDS ($\sim$ 30 mins) is substantially longer than that of \textit{word2vec} ($\sim$ 5 mins), which indicates our approach has advantages in terms of efficiency as well.
...
%
\clearpage
\todo{@dakoda, could you update the table?}
\begin{landscape}
