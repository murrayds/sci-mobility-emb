...
\end{equation}
where $v_i$ is a column vector of length $N$ with entries that are all zero except the $i$th entry that equals to one, $W = (W_{ij})$ is the weighted transition matrix of the unbiased random walker. We used $\alpha=0.9$ here.
...
Generalized gravity models use external information about each location to inform mobility predictions. 
   ...
   \end{equation}
    ...
    Similarly, $l_{ij}^x,  x \in [\text{language, language family}]$ is an dummy variable that is set to 1 if two organizations are in countries that share the same language or language family, respectively. 
