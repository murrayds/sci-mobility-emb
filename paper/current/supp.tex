...
First, we construct the co-occurrence matrix of $N$ organizations, $T_{ij}$ given by the co-occurrence of organizations $i$ and $j$ in the same affiliation trajectory. 
Then, we apply truncated singular vector decomposition with $d=300$ on the flow matrix $T$ directly.
...
We also try the embedding with the direct optimization of the gravity model.
First, we construct the gravity matrix of $N$ organizations, $T_{ij}/m_i m_j$, where $T_{ij}$ is occurrence of organizations $i$ and $j$ in the same affiliation trajectory divided by multiplication of each location's population (mean annual member of unique mobile and non-mobile author). 

Then, we try two approximation methods to get the embedding. The first method is  truncated singular vector decomposition (SVD) and the second method is a multi-dimensional scaling (MDS), which places each entity into N-dimensional space that the distance between entities preserved as well as possible.  Usually, MDS uses the euclidean distance of original vector as a distance measure.

\todo{@dakoda, please report the performace here}
