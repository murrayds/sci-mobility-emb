...
We examine the gravity model on the embedding vector with applying truncated singular vector decomposition (SVD) on the co-occurrence matrix as the first benchmark on the matrix factorization method. Intuitively,  truncated SVD performs linear low rank approximation of the matrix with given dimension $d$. First, we construct the co-occurrence matrix of $N$ organization, $A_{ij}$ given by the co-occurence of $i$ and $j$ in the same affiliation trajectory. Then, we apply truncated singular vector decomposition with $d=300$ on the flow matrix $A$ directly.

We report the result with cosine distance ($R^2=0.247$, Table~\ref{supp:table:r2_table}), which is the best performance among cosine distance, euclidean distances and dot product. The performance is slightly over the the performance of the model with geographical distance. 
\todo{@dakoda can you write down the performance with other distance measures simply here?}
...
We also consider Laplacian eigenmaps \autocite{belkin2003laplacian}, which is the most fundamental graph embedding method, as the second benchmark on the matrix factorization. First, we construct the co-occurrence matrix of $N$ organization, $A_{ij}$ given by the co-occurence of $i$ and $j$ in the same affiliation trajectory and degree matrix $D$ which is the diagonal matrix for which $D_{ii}=\sum_j {A_{ij}}$. Then we construct graph lapalcian matrix $L = D-A$  and apply  truncated singular vector decomposition in the matrix $L$ with $d=300$.

We report the result with euclidean distance ($R^2=0.341$, Table~\ref{supp:table:r2_table}), which is the best performance among cosine distance, euclidean distances and dot product. The performance is much bettern that the performance of the model with geographical distance, still below the performance of our suggested way.
\todo{@dakoda can you write down the performance with other distance measures simply here?}
...
Furthermore, We examine the gravity model with embedding vector with the symmetric SVD  \textit{word2vec} method\autocite{levy2014neural} as the third benchmark on the matrix factorization. Based on idea of \textit{word2vec} is just implciity matrix factorization, Levy proposed symmetric SVD \textit{word2vec} embedding. First, we construct the matrix of $N$ organization,
