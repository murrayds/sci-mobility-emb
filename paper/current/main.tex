...
Using the case of scientific mobility, we leverage the neural embedding model~\autocite{mikolov2013word2vec} to demonstrate a data-driven approach for learning a vector-space embedding that encodes a functional distance~\autocite{brown1970migration, brown1970functional, kim2018functional} between organizations and captures the latent and multi-faceted structure of human mobility.
We train neural embeddings of scientific organizations using mobility trajectories derived from the papers published between 2008 and 2019 of disambiguated authors from the Web of Science database.
Functional distnace is derived as the cosine distance between vectors in the resulting embedding, and is shown to better explain and predict real mobility between organizations than geographic distance. 
...
While geographic distance may have driven human movements in past eras, it holds less importance to a world connected by rapid transit, complex international relations, and global trade.
For example, a person currently living in Australia is more likely to migrate to the United Kingdom, a country with similar language and culture, than to closer and more populous countries like Indonesia. 
...
Despite its complexity, ubiquitious models of human mobility such as the Gravity~\autocite{zipf1946gravity} and Radiation~\autocite{simini2012universal} model continue to predict mobility as a function of geographic distance.
Although alternatives exist to quantify \textit{functional} distances based on factors other than geography~\autocite{boschma2005proximity, brown1970functional, brown1970migration, kim2018functional}, geographic distance remains the unambiguous and foundational quantity for modeling mobility. 
...
This approach, origionally designed for learning language models~\autocite{mikolov2013word2vec} has been used to reveal insights in text~\autocite{linzhuo2020hyperbolic, tshitoyan2019mat2vec, garg2018gender, kozlowski2018geometry, hamilton2016diachronic, e2014doc2vec, nakandala2016twitch} and networks~\autocite{perozzi2014deepwalk, grover2016node2vec}.
In addition to providing a data-driven functional distance, this approach is computationally efficient, scalable for large datasets, robust to noise, and is readily interpretable through the application of simple vector arithmetic. 

We use scientific mobility (scientific co-affiliation) as a case study to demonstrate the neural embedding approach. 
Scientific mobility is a central aspect of the increasingly-globalized scientific system~\autocite{czaika2018globalisation}, being strongly related to innovation~\autocite{braunerhjelm2020labor, kaiser2018innovation, armano2017innovation}, impact~\autocite{sugimoto2017mostimpact, petersen2018multiscale, jonkers2013return, franzoni2014advantage}, collaboration~\autocite{rodrigues2016mobility}, and the diffusion of knowledge~\autocite{braunerhjelm2020labor, azoulay2011diffusion, morgan2018prestige}.
...
Neural embedding offers a means of capturing the complex global structure of scientific mobility in a dense and coherent representation that supports modeling and predicting mobility flows, mapping  patterns of mobility at multiple scales, and quantifying abstract notions such as academic prestige. 

The most fine-grained geographical unit of scientific production is the organization, such as an university, research institute, business, or government agency.
As a scientist's careers progress, they move between organizations or pick up additional simultaneous affiliations forming co-affiliation or \textit{mobility trajectories}.
...
A vector-space embedding of organizations is learned by using mobility trajectories as input to the standard skip-gram with negative sampling, or \textit{word2vec} neural-network architecture.
In the resulting embedding, each organization corresponds to a vector, and vectors relate to one another based on the likelihood of organizations appearing near together in the mobility trajectory data. 
...
Where the \textit{expected flux}, $\hat{T_{ij}}$, or expected number of authors co-affiliated with a pair organizations during the time period is proportional to the organization's sizes (the sum of both mobile and non-mobile authors, see Data and Methods), $m_{i}$ and $m_{j}$, and decays as a function of their distance, $f(r_{ij})$.
...
Geographic distance alone is a poor perspective for understanding international scientific mobility---rather, by learning a functional distance directly from the data, it is possible to better explain, and better predict the flow of scientific mobility between organizations.
...
Why does the neural embedding model (skip-gram with negative sampling, or \textit{word2vec}), originally designed for natural language processing~\autocite{mikolov2013word2vec}, produce a reasonable distance that models gravity relationships between organizations?
We find evidence of a theoretical connection between the two models, one that makes the neural embedding model applicable to embedding many forms of mobility trajectories. The gravity model generates the flow from $i$ to $j$ by 


\begin{align}
    \label{eq:gravity_model}
    T_{ij}:= G M_i M_j f\left(d_{ij}\right),
\end{align}


where $M_i$ and $M_j$ are the masses of of the entities, $f\left(d_{ij}\right)$ is a function of the distance, and $G$ is the constant analogous to the gravitational constant in physics. 
In mobility studies, any quantity proportional to the sum of flows from $i$ can be used as $M_i$; a common choice is the population at a point $i$ (e.g., city and country).
For $d_{ij}$, geographical distance is often used. 
Here, we show that the neural embedding model can be interpreted as a generalized version of the gravity model that efficiently encodes a functional distance into the dense, continuous, and meaningful embedding vector.

In the case of language, the neural embedding model attempts to maximize the average log probability of observing a target word in a sentence, $w_{t}$, given the words in the surrounding context window, $w_{t + k}$. 
This can be written as follows, where $c$ is the size of the context window and $T$ iterates through possible target words:
...
where the conditional probability of observing one word $i$, given word, $j$, is defined as,

\begin{align}
  \label{eq:model_node2vec}
    P \left(j \mid i\right) = \frac{1}{Z_i}\exp\left(\vec{u} ^\top _{i} \vec{v}_{j}\right), 
\end{align}

where $u_i$ and $v_j$ are `the `in-vector'' and ``out-vector'' respectively, which are K-dimensional vector representations of each word in the vocabulary. 
Variable $Z_i$ is the normalization constant or the partition function, i.e., $Z_i:= \sum_{\ell=1}^N \exp\left(\vec{u} ^\top _{i} \vec{v}_{\ell}\right)$.  By the definition of the euclidean distance between the in-vectors,
...
For simplicity, we assume that  $\vec{v}_j = \vec{u}_j$, although they may differ in practice.
Exploiting the fact that $d^2 (\vec{u},\vec{u}')=||\vec{u}||^2 + ||\vec{u}'||^2 - 2\vec{u} ^\top \vec{u}'$, the dot similarly between in and our vectors $\vec{u}_i$ and $\vec{v}_j$ is given by 
...
Using Eq.~\ref{eq:dotsim}, we can define $\exp\left( \vec{u}_i ^\top \vec{v}_j \right)$ as
...
\begin{align}
    ...
    P \left(j \mid i\right) &:= \frac{1}{Z_i}\cdot \frac{s_is_j}{\exp\left[d\left(\vec{u}_i, \vec{u}_j\right)^2/2\right]}. 
\end{align}

If we consider a stable Markov process with the transition probability from $i$ to $j$, $P(j \vert i)$ and consider the stationary distribution for the Markov process  $\pi$, i.e., mobile scientists. Then, the flow from $i$ to $j$  in one step is given by 

\begin{align}
    \label{eq:flow}
    T_{ij}:= \pi_i P(j \vert i).
\end{align}

According to detailed balanced condition of Markov process, 

\begin{align}
    \pi_i P\left(j \vert i \right) = \pi_j P\left(i \vert j \right),
\end{align}
...
\begin{align}
    ...
    T_{ij} &= \pi_i P\left(j \vert i \right)\nonumber \\
...
The form of Eq.~\ref{eq:flow_gravity_model} maps to the gravity model $T_{ij}:= G M_i M_j f\left(d_{ij}\right)$.
Therefore, we argue that the nerual embedding model can be interpreted as a generalized version of the gravity model with $M_i=s_i$ and $f\left(d_{ij} \right) = \exp\left[ - d\left(\vec{u}_i,\vec{u}_j\right) ^ 2 /2\right]$.
Given flows $T_{ij}$ derived from mobility trajectories, neural embedding model attempts to learn the mass $M_i$, map each entity, and encode the functional distance into more abundant vector space . 
Hence, the neural embedding model can be an effective model for investigating any kind of mobility trajectory data, stemming from the mathematical relationship between it and the gravity model of mobility.

%
>>>>>>> bcf30915179394422cad63d1bd7fa0d62e0367b1
% Subsection: Applications
%
\subsection*{Embeddings capture global structure of mobility}

What information is encoded in the embedding that allows embedding distance to outperform geographic distance?
To explore the structure of the embedding space, we use a common dimensionality reduction technique (UMAP~\autocite{mcinnes2018umap}) to visualize it in two dimensions (Fig.~\ref{fig:projection}a).
...
At the same time, the embedding space also reflects a mix of geographic, historic, cultural, and linguistic relationships between regions much more clearly than alternative network representations (Fig.~\ref{fig:supp:network_vis}) that have dominated studies of scientific mobility~\autocite{chinchilla2018global, czaika2018globalisation}.

An advantage of the embedding space is the ability to \textit{zoom in} on subsets of counties and re-project them in order to reveal more local relationships. 
For example, re-projecting organizations located in Western, Southern, and Southeastern Asia (Fig.~\ref{fig:projection}b) reveals a gradient of countries ranging from Egypt to Phillipines that corresponds to both geography but also cultural factors, such as the majority religion of each country;
for example, the cluster for Malaysia, a majority-Muslim country, is nearer to other Middle Eastern nations than Thailand, a country that is geographically-nearer though with very different religious demographics. 
...
For example, at the global level (Fig~\ref{fig:projection}a), organizations within Quebec in Canada are located near the French cluster rather than the rest of Canada.
...
The six identified clusters roughly correspond to: East-Asian (orange), Scandinavian (dark purple), British Commonwealth (light purple), Central and Eastern European countries (light blue), and then two cluster diverse clusters with a mix of European, Latin American, and Mediterranean countries.
...
We test this by exploting the geometric properties of the embedding space with the technique SemAxis~\autocite{an2018semaxis}, an approach originally designed for characterize the semantics of words in language embeddings, but is equally applicable here. 
In the context of organization vectors, this approach leverages the unique properties of the embedding space in order to quantify the nearness of an organization to the ends of some meaningful axis. 
The axis is defined by the user and should represent some meaningful  and coherent concept. 
Organization vectors are then projected onto this axis, and their position estimates their similarity to the concepts embodied by either end of the axis. 
...
Accordingly, prestigious universities such as Stanford, Colombia, MIT, Harvard, and NYU are positioned towards the top of the axis (Fig.~\ref{fig:semaxis}a).
Universities at the bottom of this axis tend to be regional universities with lower national profiles.
By considering mobility between all organization types, SemAxis applied to embeddings offers a new means of reasoning about the prestige of organizations typically excluded from rankings such as government organizatioss (Fig.~\ref{fig:supp:semaxis_sectors}a), research institutes (Fig.~\ref{fig:supp:semaxis_sectors}b), and teaching colleges (Fig.~\ref{fig:supp:semaxis_sectors}c).
...
\begin{figure}[hp!]
	...
	\caption{
		...
		\textbf{a.} illustrates SemAxis projection along two axes, comparing California to Massachusetts universities (left to right), and between the top five and geographically-matched bottom five universities ranked by the 2018 Times Higher Education ranking of U.S. Universities (bottom to top).
		...
		White points are those top and bottom five universities aggregated to define the ends of the axis.
		The grey box corresponds to the top five and bottom five ranks.
...
The embedding space captures prestige hierarchies to such a high resolution that they can be used to reconstruct academic rankings. 
...
We find that SemRank derived is highly correlated with the Times ranking ($\rho = 0.73$, Fig.~\ref{fig:semaxis}b).
...
This suggests that the organization embedding captures the latent prestige hierarchy inherent to academic mobility with a high degree of resolution and reliability. 

While we use SemAxis to operationalize notions of prestige and geography, the approach can be easily generalized to other concepts so long as they can be meaningfully represented as an axis in the vector space. 
For example, axes could similarly be defined between teaching- and research-focused universities, between companies and government agencies, or between organizations in the U.S. and in China. 
...
Mobility and migration are central to the human experience~\autocite{shah2020migration}, but recent events threaten to upend patterns of human mobility that defined recent decades. 
Within science, mobility has grown increasingly global~\autocite{czaika2018globalisation}, though the travel restrictions in the U.S. and U.K. brought by Brexit and the Trump administration~\autocite{chinchilla2018travelban}, and the rise of new scientific powers such as China~\autocite{tollefson2018china, cao2020returning} may lead to reorientation in who goes where.  
...
Neural embeddings offer a natural approach for learning an effective and robust functional distance between places based on mobility trajectory data. 
...
Moreover, this approach can be used to learn a functional distance between entities when no such analog to geographic distance exists, such as a functional distance between occupational categories based individual's career trajectories. 
In addition to providing a functional distance that supports modeling and predicting mobility patterns, the unique structure of neural embedding also allow for a range of methodological applications.
For example, projecting the embedding space onto two dimensions using UMAP allows for the visualization of the complex structure of scientific mobility at high resolution and at multiple scales. 
Other techniques, such as SemAxis, allow the the potential to quantify abstract notions in mobility such as academic prestige. 
Neural embedding open the door for new approaches for representing and understanding patterns of global human mobility. 
