...
Then, we can rewrite the \ref{eq:gravity_model},
\begin{equation}
...
where $v_w$ and $v'_w$ are ``input'' and ``output'' vector representations of each word in the vocabulary.  We use Skip-gram negative sampling, and following the work by Levy and Goldberg \cite{levy2014neural}, it is implicitly factorizing 

\begin{equation}
    \label{eq:sgns_relation}
    \log \left(\frac{\#(w,c) |D|}{\#(w) \#(c)} \right) - \log k
\end{equation}
where $\#(w)\text{,} \#(w) \text{ and } \#(c)$ denote the number of times word w, c and word-context-pair (w,c) in the corpus respectively. $k$ is the number of negative samples and $|D|$ is the size of the corpus. Eq. \ref{eq:sgns_relation} is symmetry matrix, result vector representation  $v_w$ and $v'_w$ are identical. With these relations, for a specific pair of words $(w_i, w_j)$,
...
$|D|$ and k is constant, then it can be rewritten as
