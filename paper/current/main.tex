...
The SGNS model learns an effective distance from trajectories, one that we visualize whole organization vectors in  Fig. \ref{fig:projection}a, revealing organizational clusters that reflect a mix of geographic, historic, cultural, and linguistic relationships between countries.
The coordinates of each organization come from a 2-D projection of the embedding vectors with UMAP \cite{mcinnes2018umap} and the colors indicate the continent that the organization belongs to. The overall structure is almost similar to the geographical map seen from the north pole while coherent structures of the communities are detected. For instance, french-spoken regions of Canada (Quebec, Montreal) are more close to the French cluster rather than the mainland of Canada, which is the result of the linguistic relationship. Also, the cluster of Algeria and Morocco is closed to the French cluster, which is far from the mainland of Africa cluster, which reflects colonial historical facts.

To look at the multi-scale structure of the embedding closely, we re-project organization vectors using a subset of embedding vectors, which is another advantage of our proposed method. In Fig. \ref{fig:projection}b, we find the cultural, religious, and regional spectrum of the country clusters across the Western, South, Southeast Asia. Also, we observe the clear linguistic separation of the country cluster, which are Portuguese speaking countries (Brazil, Portugal) and Spanish speaking countries (Spain, Mexico, Chile, etc.)  in Fig. \ref{fig:projection}c.

By visualizing subsets of vectors within a specific country, we reveal a more granular structure. We re-project the embedding vectors within the United States in Fig. \ref{fig:projection}d, find state-level geographical clustering the structure of organizations roughly organized by state and region with spectrum from the East-coast to the West-coast. The more interesting part is re-projection results on state levels. In Fig. \ref{fig:projection}e, we re-project the embedding vectors with only organization in Massachusetts, finding structure based on urban centers (Boston vs. Worcester), organization type (hospitals tend to be adjacent)
and different university systems (UMass system cluster separate from Harvard, MIT, etc.). Even though UMass Boston is located in Boston, it clusters with other universities in the UMass System and it observed throughout many states (New York, California, etc.) with state university system (See SI~?).
...
Based on findings on visualizing results, we try to capture the international relationship of the cluster in a more quantitative manner. We calculate the representative vector of each country with averaging organization vectors within the given country, calculate pairwise cosine similarity and do the hierarchical clustering (see Fig. \ref{fig:clustering}a).
Countries that have more than 25 organizations are collected for the clustering analysis, excluding the United States because all countries are closed to the United States.
We report the similarity matrix, deprogram and meta-data of each country.
Color of the country name is colored by the cluster and the color of the three cell rows represents the continent, language, and language family respectively. We assign a single language to country with the largest used language based on language demographic data \cite{ethnologue}.

We found 6 clusters, which is a mixture of the regional, linguistic, historical clusters. For instance, the purple cluster is well-known commonwealth countries and the lower part of the sky-blue clusters is post-soviet countries, reflecting historical relationships among countries. Also, the upper part of the sky-blue clusters is the linguistic cluster.  By looking closely at the deprogram and meta-data, there is a tendency that countries are merged as a group if they share the language in the early stage of the deprogram and geographical aspect plays a more major role in the later stage of the dendrogram.

We measure this phenomena quantitatively with \textit{CluSim} method \cite{gates2019element}. \textit{CluSim} is a method that calculates the similarity between the cluster results and, for the hierarchical cluster, it can adjust the relative importance of membership at different levels of the hierarchy with scaling parameter r, acting likes ``zooming lens''.  If r is high, the similarity is based on the lower part of the deprogram and the similarity is based on higher part of the deprogram with low r. 
We identify the dominant factor that determines the hierarchy by comparing hierarchical results and each meta-data membership in each level, leveraging characteristic of \textit{CluSim}.
...
As expected, the continent plays a major role at higher levels of clustering process (low r), language and language family explain the clustering process more at the lower levels (high r). It suggests that language is quite an important factor for scientific mobility, researchers tend to moves to the country that uses the same language or belongs to the same language family at least.
