...
We argue that the neural embedding model
Here, we show that the neural embedding model can be interpreted as generalized version of the gravity model that efficiently encodes a functional distance into the dense, continuous, and meaningful embedding vector.
...
Therefore, we argue that the nerual embedding model is a generalized version of the gravity model with $M_i=s_i$ and $f\left(d_{ij} \right) = \exp\left[ - d\left(\vec{u}_i,\vec{u}_j\right) ^ 2 /2\right]$.
Given flows $T_{ij}$ derived from mobility trajectories, this model attempts to learn the mass $M_i$ and map each entity into more abundant vector space . 
...
What kind of information is encoded in the embedding that allows embedding distance to outperform geographic distance?
...
This projection constitutes the largest and highest resolution ``map'' of scientific mobility to date.
...
For example, re-projecting organizations located in Western, Southern, and Southeastern Asia (Fig.~\ref{fig:projection}b) reveals a gradient of countries ranging from Egypt to Phillipines that correponds to both geography but also cultural factors, such as the majority relitigion of each country;
...
Just as the global embedding captures national-level geographic structure, so does a projection of organizations in the United States reflect organizational structure based on states and regions (Fig.~\ref{fig:projection}d).
...
We test this by exploting the geometric properties of the embedding space with the technique SemAxis~\autocite{an2018semaxis}, an approach originally designed for word embeddings but that is equally applicable here. 
SemAxis assumes the existence of an a user-defined axis, defined as the vector formed by the distance of some two vectors in the embedding space.
The choice of the axis should represent a meaningful and coherent concept.
...
Accordingly, prestigious universities such as Stanford, Colombia, MIT, Harvard, and NYU, group towards the top of the axis (Fig.~\ref{fig:semaxis}a).
...
The embedding space captures prestige hierarchies to such a high resoltuion that they can be used to reconstruct academic rankings. 
