...
Here, we demonstrate a data-driven approach of learning an effective distance that encodes the latent structure of mobility using a neural embedding model~\autocite{mikolov2013word2vec} trained on mobility traces sourced from the publications of disambiguated authors from the Web of Science database between 2008 and 2019.
...
With this approach, we can learn scalable, dense, and meaningful vector representations of scientific mobility directly from the data using a methodology that can be easily applied to all kinds of mobility.
\end{abstract}
...
When scientists move, they bring their knowledge with them.
For scientific mobility, like all human movement, distance matters.
However, how exactly do we measure the distance between two places?
Although many alternative ways to quantify the distance~\autocite{boschma2005proximity} exist, geographic distance remains an unambiguous and foundational quantity for the distance.
...
Here, we propose a neural embedding model as an effective means of encoding an effective measure of distance using massive trajectory data.
...
By treating geo-located organizations analogously to words and trajectories analogously to sentences, we learn vector-space embedding of organizations using neural embedding (Fig.~\ref{fig:methods:mobility_sentence}).
The resulting vector of organizations is representation that the \textit{cosine distance} between vectors represents the distance in the landscape of scientific mobility.
...
Where the flux, $T_{ij}$, or number of authors co-affiliated with a pair organizations during the time period increases with the organization's sizes, $m_{i}$ and $m_{j}$, and decays as a function of their distance, $f(r_{ij})$.
...
\begin{figure}[h!]
	...
	\caption{
		...
		\textbf{e-f.} Predictions made using model parameters learned from subset of domestic (within-country) organization pairs.
...
Hence, the skip-gram word embedding model can be an effective model for investigating any kind of mobility trajectory data, stemming from the mathematical relationship between it and the gravity model of mobility.
...
Word embeddigns produces an effective distnace from mobility which can be visualized by projecting the embedding space into two dimensions using UMAP~\autocite{mcinnes2018umap} (Fig.~\ref{fig:projection}a).
...
In addition to learning geography, its clear that the word embedding also encodes the latent prestige hierarchies of academic mobility.
...
For reference, SemRank achieves correlation with the Times Ranking comparable to that between the Times ranking and the Leiden Ranking mean-normalized citation score ($\rho = 0.87$, Fig.~\ref{fig:supp:times_vs_leiden}).
This suggests that word embeddings, in addition to learning the latent geography of mobility, while capture more nuanced characteristics such as latent hierarchy in the axis of the embedding, which is fascinating characteristic of our approach.
...
By learning a distance directly from the data, word embeddings successfully  and efficiently captures a robust measure of semantic distance between entities, all with existing tools and minor data processing.
