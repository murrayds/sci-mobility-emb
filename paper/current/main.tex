...
Let $T_{ij}$ represents the \textbf{flux}, the number of people who moved between organizations $i$ and $j$. $M_{i}$ and $M_{j}$ represent the size of an organization. According to our gravity model on the embedding space,
...
It can be checked in Fig.\ref{fig:math_part}. With this approximation, we can rewrite Eq.~\ref{eq:gravity_model} as,
\begin{equation}
...
In or model, $\#(w_i)$ and $\#(w_j)$ are total flux $M_i$ and $M_j$ respectively,  and $\#(w_i,w_j)$ is just  $T_{ij}$, flux between i and j. Then, we have same expression with gravity model,
