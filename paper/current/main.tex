...
\begin{figure}[hp!]
	...
	\caption{
		...
		\textbf{c.} Re-projection of organizations in Spain, Portugal, and the Portuguese- and Spanish-speaking counriees in South and Central America. 
...
\begin{figure}[p!]
	...
	\caption{
		...
		\textbf{a.} Hierarchically clsutered similarity matrix of country vectors, aggregated as the mean of all organization vectors within the country, excluding the United States. 
		...
		\textbf{b.}	\textit{CluSim} results demonstrating the factors dictating hierarchical clustering, varying by scaling parameter, $r$. 
...
% $p(j|i)$ and ${\mu_j}$ is given from data, and the our model trains $\exp(\cos(v_i,v_j))$ with the data. Then, can we infer the functional form of the $p(j|i)$ which is written with the results of the model?

% Cosine similarity capture the relative importance of neighborhood, because, with same $p(j|i)$, if $\mu_j$ is large, $\exp(\cos(v_i,v_j)$ should small and if $\mu_j$ is small, $\exp(\cos(v_i,v_j)$ should be large. Based on this relationship, cosine similarity capture the relative importance. Simply, we can use $p(j|i)$ as just normalized importance with

% Also, we can validate this proxy with checking inverse relationship, \ref{eq:inverse_relationship}.

% \begin{equation}
%     \frac{f_{ij}}{\mu_i * \mu_j} \propto \frac{\exp(\cos(v_i, v_j))}{\mu_i * \sum_{w\in\mathpzc{N}_j} \exp(\cos(v_w,v_j))} \propto \exp(\cos(v_i,v_j))
% \end{equation}
% \begin{equation}
% \label{eq:inverse_relationship}
%      \mu_i * \sum_{w\in\mathpzc{N}_j} \exp(\cos(v_w,v_j)) \propto C
% \end{equation}


% According to Fig. \ref{fig:inverse_relationship}, $\mu_{i}$ and $\sum_{w \in \mathcal{N}_{j}} \exp(\cos(v_w \cdot v_j))$ have either a negative power-law relationship, or a weak inverse relationship. 
% This relationship may result form hub nodes, or major organizations central to the global network of co-affiliations. 
% A node with large $\mu_{i}$ would be considered a hub node in the network, and following the friendship paradox, neighboring nodes will likely have fewer neighbors than the hub. 
...
%     \label{fig:inverse_relationship}
% \end{figure}
