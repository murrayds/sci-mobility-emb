...
Performance is also comparable to network-driven measures such as degree strength (Spearman's $\rho = 0.73$) and eigenvector centrality (Spearman's $\rho = 0.76$, see Supporting Information). 
...
The embedding-ranking of U.S. universities is less correlated with weighted degree (Spearman's $\rho = 0.45$, Fig.~\ref{fig:supp:centrality_semaxis_compare}a) and eigenvector  centrality (Spearman's $\rho = 0.55$) than with the Times Ranking itself (Spearman's $\rho = 0.73$, Fig.~\ref{fig:supp:centrality_semaxis_compare}b). 
