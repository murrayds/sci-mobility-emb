...
Here, using a rich dataset of scientific mobility, we demonstrate that a data-driven neural embedding~\autocite{mikolov2013word2vec} can successfully encode a functional distance between organizations and capture the latent and multi-faceted structure of scientist's mobility.
...
We demonstrate that the embedding space encodes not only geographic relations between organizations, but also relations based on culture, language, and academic prestige at multiple levels of granularity.
The embedding space also captures the relationship between organization's size and their connectedness that repeats across most countries.
...
They are also computationally efficient, robust to noise, and can encode reasonably rich relations between entities as geomemetric relationships in the vector space~\autocite{levy2014neural, nakandala2017gendered, kozlowski2018geometry, an2018semaxis}.


We use scientific mobility, inferred from researcher's affiliations, to demonstrate the effectiveness of our approach.
...
In the resulting embedding, each organization corresponds to a single vector, and vectors relate to one another based on the likelihood of organizations appearing adjacent to one another in the same affiliation trajectory.
...
\end{equation}
where the \textit{expected flux}, $\hat{T_{ij}}$, or expected number of authors who were affiliated with both organizations during the time period, is proportional to the organizations' sizes (the sum of both mobile and non-mobile authors, see Data and Methods), $m_{i}$ and $m_{j}$, and decays as a function of their distance, $f(r_{ij})$.
...
To explore the topological structure of the embedding, we use a standard dimensionality reduction technique (UMAP~\autocite{mcinnes2018umap}) to obtain its two-dimensional representation (Fig.~\ref{fig:projection}a).
...
For example, re-projecting organizations located in Western, Southern, and Southeastern Asia (Fig.~\ref{fig:projection}b) reveals a gradient of countries between Egypt and the Philippines that corresponds largely to geography, but with some peculiar exceptions along the cultural and religious spectrums.
For example, Iraq is nearer to some southeast-Asian countries such as Malaysia, with a cosine distance between the nation's aggregate vectors of $d = 0.29$, than to closer and more religiously-similar nations such as Saudi Arabia ($d = 0.50$) and Qatar ($d = 0.51$).
Linguistic and historical ties are also conspicuous.
For example, we observe that Spanish-speaking post-colonial Latin American nations are positioned near Spain (Fig.~\ref{fig:projection}c), rather than nearby Portuguese-speaking nations ($d = 0.34$ vs. $d = 0.52$ for Mexico and $d = 0.39$ vs. $d = 0.49$ for Chile) reflecting linguistic and cultural ties.
Similarly, North-African countries that were once under French rule such as Morocco are nearer to France ($d =0.35$) than to similarly-distant European countries such as Spain ($d = 0.40$) and Italy ($d = 0.53$).
...
By re-projecting Massachusetts (Fig.~\ref{fig:projection}e), we observe structure based on: urban centers (Boston vs. Worcester), organization type (e.g., hospitals vs. universities) and university systems (UMass vs. Harvard \& MIT).
...
Prestige hierarchy is known to underpin the dynamics of scientific mobility~\autocite{deville2014career, clauset2015hierarchy}, but can the hierarchy be encoded in the embedding space, to which no explicit information on prestige is given and for which the direction of mobility is not known?
...
The axes can further be visualized to examine the relative position of organizations along the prestige axis, and along a geographic axis between California and Massachusetts that aids visualization.
...
Up to a point (around 1,000 researchers), the size of U.S. organization's vectors tends to increase proportional to the number of researchers (both mobile and non-mobile) within that organization with published work;
...
This same convex-curve repeats across many countries such as China, which closely mirrors that of the United States (Fig.~\ref{fig:length}b).
...
\begin{figure}[h!]
	...
	\caption{
		...
		A convex-shape emerges, wherein larger universities tend to be more distant from the origin (large L2 norm); however, the more prestigious the university, the closer it is to the origin (small L2 norm).
...
The kinds of mobility we do capture—migration and co-affiliation, are treated identically by our model even though they are qualitatively different.
...
Traditional hubs of migration have experienced many politially-motivated policy changes that affect scientific mobility, such as travel restrictions in the U.S. and U.K.~\autocite{chinchilla2018travelban}.
...
Unprecedented health crises such as COVID-19 pandemic threaten to bring drastic global changes to travel and migration by tightening borders and halting travel.
