...
We show that the embedding distance, compared with the geographical distance, predicts actual mobility flows. 
In the case flight itineraries, the embedding distance explains more than twice the expected flux between airports ($R^{2} = 0.51$, Fig.~\ref{fig:gravity_performance}a) than does geographic distance ($R^{2} = 0.22$, Fig.~\ref{fig:gravity_performance}b). 
...
The embedding distance produces better predictions of actual flux between airports than does the geographic distance (Fig.~\ref{fig:gravity_performance}c-d).
These results are consistent in the case of scientific mobility. 
the amount of variance in the expected flux explained by the embedding distance ($R^{2} = 0.48$, Fig.~\ref{fig:gravity_performance}e) is again more than double that explained by the geographic distance ($R^{2} = 0.22$, Fig.~\ref{fig:gravity_performance}f).
...
As with flight itineraries, we fit the gravity model on the scientific mobility data using the gravity model.
Predictions made using the embedding distance outperform those made using the geographic distance (Fig.~\ref{fig:gravity_performance}g-h), which also holds for the subsets of only domestic and only international mobility (Fig.~\ref{fig:supp:predict_geo_bymodel} and Fig.~\ref{fig:supp:predict_emb_bymodel}).
For scientific mobility, the embedding distance defined from the cosine distance between vectors also outperforms that defined from their dot product ($R^{2} = 0.34$, Fig.~\ref{fig:supp:gravity_dot}).
...
\begin{figure}[h!]
	...
	\caption{
		\textbf{Embedding distance encodes functional distance and better predicts U.S flights and global scientific mobility.}
		\textbf{a-b.} Embedding distance (cosine distance between organization vectors) better explains the expected flux of passengers between U.S. airports than does geographic distance. 
		...
		\textbf{c-d.} Predictions of flux between airport pairs made using embedding distance outperform those made using geographic distance. 
		...
		\textbf{e-f.} Embedding distance better explains the expected flux of researchers between organization pairs than geographic distance.
		\textbf{g-h.} Predictions of flux between organization pairs made using embedding distance outperform those made using geographic distance.
	}
