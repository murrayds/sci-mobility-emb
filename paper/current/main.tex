...
We find evidence of a theoretical connection between the two models, one that makes the neural embedding model applicable to embedding many forms of mobility trajectories. 
...
\end{align}
where $M_i$ is the mass for the gravity model, $f\left(d_{ij}\right)$ is a function of the distance, and 
$G$ is the constant analogous to the gravitational constant in physics. In migration study, any quantity that proportion to sum of flow from $i$ can be used as $M_i$, often population at a point $i$ (e.g., city and country) is popular choice. For the $d_{ij}$, geographical distance is often used. In other words, the gravity model inference most appropriate  $f\left(d_{ij}\right)$ with given $M_i, M_j, T_{ij},$ and $d_{ij}$. Here, we are going to show that \textit{word2vec} can be interpreted as generalized version of the gravity model, which preserved the original information into the scalable and meaningful embedding vector.

Word embedding model \textit{word2vec} attempts to maximize the average log probability of observing a target word in a sentence, $w_{t}$, given the words in the surrounding context window, $w_{t + k}$. This can be written as follows, where $c$ is the size of the context window and $T$ iterates through possible target words:
...
\end{align}
where $u_i$ and $v_j$ are ``in-vector'' and ``out-vector'' respectively, which is K-dimensional vector representations of each word in the vocabulary.  Variable $Z_i$ is the normalization constant or the partition function, i.e., $Z_i:= \sum_{\ell=1}^N \exp\left(\vec{u} ^\top _{i} \vec{v}_{\ell}\right)$.  By the definition of the euclidean distance between the in-vectors,
\begin{align}
    \label{eq:distance}
    d(\vec{u},\vec{u}'):= \sqrt{\sum_{k=1}^K \left(u_{k} - u'_{k}\right)^2}.
\end{align}

For simplicity, we assume that the $\vec{v}_j = \vec{u}_j$, it can be different practically. Exploiting $d^2 (\vec{u},\vec{u}')=||\vec{u}||^2 + ||\vec{u}'||^2 - 2\vec{u} ^\top \vec{u}'$, the dot similarity for $\vec{u}_i$ and $\vec{v}_j$ is given by 
\begin{align}
    \label{eq:dotsim}
...
\end{align}
where $s_i:=\exp(||\vec{u} _i||^2/2)$. Substituting Eq.~\eqref{eq:exp_distance_form} into Eq.~\eqref{eq:model_node2vec} yields
\begin{align}
...
If we consider a stable Markov process with the transition probability from $i$ to $j$, $P_(j \vert i)$ and consider the stationary distribution for the Markov process  $\pi$, i.e., mobile scientists. Then, the flow from $i$ to $j$  in one step is given by 
\begin{align}
    \label{eq:flow}
...
Remind that $T_{ij}:= G M_i M_j f\left(d_{ij}\right)$ in the gravity model.
Thus, the word2vec is the generalized version of the gravity model with $M_i=s_i$ and $f\left(d_{ij} \right) = \exp\left[ - d\left(\vec{u}_i,\vec{u}_j\right) ^ 2 /2\right]$, which automatically learns the mass $M_i$ and map each entity into more abundant vector space with only given $T_{ij}$. Hence, the neural embedding model can be an effective model for investigating any kind of mobility trajectory data, stemming from the mathematical relationship between it and the gravity model of mobility.
