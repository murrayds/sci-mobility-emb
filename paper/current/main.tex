...
We find that the length of organization vectors--- the distance from the origin of the vector space---provides insight into the characteristics of organizations. 
...
However, at around 1,000 researchers, the length of the vector decreases with increment in the number of researchers;
...
This same convex-curve repeats irrespective of countries, such as China and Germany, though the curve may be shifted based on the typical size and vector length of their organizations (Fig.~\ref{fig:length}b). 

This pattern has been observed in applications of neural embedding on natural language, wherein the length of word vectors was found to represent the word "significance"~\autocite{schakel2015measuring}
...
At the national level, the relative position of the convex-curves can reveal characteristics of their scientific organization. For instance, Chinese organizations have a longer average and typical vector length ($\overline{l}=4.81 $, Fig.~\ref{fig:length}b) compared to organizations in United States ($\overline{l}=4.21$), reflecting lower overall connectedness in the embedding space. 
Prestige is a key component of academic mobility—by encoding information relating to prestige, neural embeddings offer a means of better representing the functional distance between organizations and capturing the global landscape of mobility. 
