...
The embedding distance also out-performs alternative ``diffusion'' network distance measures such as the personalized-page rank scores (see supporting materials) calculated from the underlying mobility network (Fig.~\ref{fig:supp:gravity_pprcos}, Fig.~\ref{fig:supp:gravity_pprjsd}).
Our results unequivecolly argue that the distance learned by the embedding method better capture actual mobility patterns than does geographic distance. 
...
\begin{figure}[h!]
	...
	\caption{
		...
		``RMSE'' is calculated as the root-mean-squared error between the actual and predicted values.
...
To explore the embedding, we use a common dimensionality reduction technique (UMAP~\autocite{mcinnes2018umap}) to obtain a two-dimensional representation of the embedding (Fig.~\ref{fig:projection}a).
...
for example, the cluster for Malaysia, a majority-Muslim country, is nearer to other Middle Eastern nations than Thailand, a country that is geographically-nearer but religiously-distant. 
Linguistic and historical ties are also conspicuous.
For example, we observe that Spanish-speaking post-colonial Latin American nations (Mexico, Chile, Peru, etc.) are positioned near Spain, whereas Brazil is nearer to Portugal (Fig.~\ref{fig:projection}c) reflecting the role of language, culture, and colonial history that shapes scientific mobility.
...
\begin{figure}[hp!]
	...
	\caption{
		...
		White points are those top and bottom five universities used to define the poles of the axis.
	  	Even when considering only a total of ten organization vectors, the estimate of the spearman rank correlation between the SemAxis and Times ranking is 0.73 (n = 145), and increases if controlling for the number of top-and-bottom ranked universities (Fig.~\ref{fig:supp:semaxis_compare}).
	}
	\label{fig:semaxis}
\end{figure}
