...
To rewrite  Eq.\ref{eq:gravity_model} in terms of $T_{ij}$, we replace  $M_*$ with $\mu_{i*}$, the total flux to or from $i$ which can be written as
...
where $N(i)\text{, } N(j) \text{ and } N(i,j)$ denotes the number of times word $i$, $j$, and word-context-pair ($i$, $j$) in the corpus respectively. Variable $k$ is the number of negative samples and $|D|$ is the size of the corpus.

From Matrix Eq.\ref{eq:sgns_relation}, we can get embedding vector with rank-d approximation by SVD, $A=L_d \Sigma_d R_d$.
Mathematically, the maximization of the objective function of the model yields $V=L_d \sqrt{\Sigma_d}$ and $V'=\sqrt{\Sigma_d}R_d$ where L and R are the left and right singular vectors of the matrix Eq.\ref{eq:sgns_relation}, respectively. Note that $V=L_d \sqrt{\Sigma_d}=\sqrt{\Sigma_d}R_d=V'$ because Eq.\ref{eq:sgns_relation} is a real symmetric matrix. With these relations, for a specific pair of words $(i ,j )$,
