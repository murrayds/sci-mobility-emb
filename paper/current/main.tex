...
The embedding distance produces better predictions of actual flux than does geographic distance (Fig.~\ref{fig:gravity_performance}c-d), which also holds for the subsets of only domestic and only international mobility (Fig.~\ref{fig:supp:predict_emb_bymodel} and Fig.~\ref{fig:supp:predict_geo_bymodel}).
...
Our results unequivocally demonstrate that the embedding distance better captures patterns of actual scientific mobility than does geographic distance.

This approach is generalizable to other domains, such as learning a functional distance between airports. 
We learned an embedding of U.S. airports following a similar procedure as scientific mobility, using trajectories formed by the flight itineraries of individual passengers (see Data and Methods).
As with scientific mobility, the embedding approach better predicts real-world flows of airline passengers than does geographic distance. 
Embedding distance explains more of the expected flux between U.S. airports ($R^{2} = 0.51$, Fig.~\ref{fig:gravity_performance}e) than does geographic distance ($R^{2} = 0.22$, Fig.~\ref{fig:gravity_performance}f). 
Fitting the data to the gravity model given by Eq.~\ref{eq:gravity_basic} using each of embedding and geographic distance as parameters, and we find that predictions made using embedding distance out-performed those made using geographic distnace (Fig.~\ref{fig:gravity_performance}g-h). 
These results demonstrate that embeddings are applicable to a diverse range of mobility trajectory data. 
