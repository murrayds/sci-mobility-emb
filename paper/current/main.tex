...
Here, we propose a neural embedding model as an efficient means of encoding an effective measure of distance using massive trajectory data.
...
Where the \textit{expected flux}, $\hat{T_{ij}}$, or expected number of authors co-affiliated with a pair organizations during the time period increases with the organization's sizes, $m_{i}$ and $m_{j}$, and decays as a function of their distance, $f(r_{ij})$.
