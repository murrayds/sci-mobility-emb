...
These trajectories encode both migration and co-affiliation---the holding of multiple simultaneous co-affiliations involving the sharing of time and capital between locations---that is typical of scientific mobility~\autocite{rodrigues2016mobility, markova2016synchronous, sugimoto2017mostimpact} (see Supplementary Materials).
...
The performance of the embedding ranking in reconstructing the Times ranking is comparable to that of network-derived measures such as weighted degree (Spearman's $\rho = 0.73$, Fig.~\ref{fig:supp:centrality_times_compare}a) and eigencentrality centrality (Spearman's $\rho = 0.76$, Fig.~\ref{fig:supp:centrality_times_compare}b).
...
The embedding-ranking of U.S. universities is less correlated with weighted degree (Spearman's $\rho = 45$, Fig.~\ref{fig:supp:centrality_semaxis_compare}a) and eigenvector  centrality (Spearman's $\rho = 0.55$) than with the Times Ranking itself (Spearman's $\rho = 0.73$, Fig.~\ref{fig:supp:centrality_semaxis_compare}b). 
...
This suggests that the embedding encodes encodes information on prestige hierarchy at least as well as a network representation, but that the information encoded is something different from centrality.
...
By projecting other types of organizations onto the prestige axis, SemAxis offers a new means of reasoning about the prestige of organizations typically excluded from rankings such as government organizations (Fig.~\ref{fig:semaxis}d), research institutes (Fig.~\ref{fig:semaxis}e), and teaching colleges (Fig.~\ref{fig:semaxis}f).
...
This same convex-curve repeats across many countries (Fig.~\ref{fig:concave30})., though with variations that can reveal characteristics about the scientific systems of each country (Fig.~\ref{fig:length}b).
For example, the representative vector of Chinese organizations are larger than that of the United States ($\bar{l} = 2.97$ vs $\bar{l} = 2.39$, Table~\ref{table:supp:norm}), reflecting the tendency for organizations in the United States to appear more frequently in more different contexts than Chinese organizations.
...
Scientific mobility is complex and can be defined in many ways~\autocite{robinson2019mobility} and at multiple levels of analysis.
...
\textit{word2vec} constructs dense and continuous vector representations of words and phrases, in which distance between words correspond to a notion of semantic distance. There are two types of embedding vectors in the model; in-vector and out-vector. In-vector is usually mainly used as a representation vector of each word \autocite{linzhuo2020hyperbolic, tshitoyan2019mat2vec, garg2018gender, kozlowski2018geometry, hamilton2016diachronic, le2014doc2vec, nakandala2016twitch} and we also used in-vector in this study.
By embedding affiliation trajectories, we aim to learn a dense vector for every organization where the distance between vectors relates to the tendency for two organizations to occur in similar contexts.
Embedding vectors are learned by training a neural network to following average log probability using as input an affiliation trajectory---a sequence of tokens identifying organizations $a_{1}, a_{2}, ... a_{T}$,

$$
\frac{1}{T}\sum_{t = 1}^{T} \sum_{-c \leq j \leq c, j \neq 0} \log p(a_{t + j} \vert a_{t})
$$

where T is the total size of the training sequence, $c$ is the window size before and after the target organization token $a_{t}$ that is used to define the context, $a_{t + j}$~\autocite{mikolov2013word2vec}. 
...
The skip-gram negative sampling model has several tunable hyper-parameters, including the dimension of the learned vectors, the size of the token window considered when training, the minimum frequency threshold for including an organization, and the number of iterations.
...
Negative cosine distances, caused by cosine similarities greater than one, are truncated at zero. 
Note that this distance derived from the embedding space is not a formal metric, as it does not satisfy the triangle inequality, however such distnaces have still proved practically useful in make tasks~\autocite{lerman2007embedding, brown1970migration, brown1970functional, kim2018functional}. 
