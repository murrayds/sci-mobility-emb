...
In the resulting embedding, each organization corresponds to a vector, and vectors relate to one another \textit{semantically}--just as with language, organizations are \textit{semantically} similar when they share similar co-affiliation \textit{contexts}. 
We measure the semantic distance between organization vectors using \textit{cosine distance}, and use this to model distance in the landscape of scientific mobility. 
