...
As we shown, projecting the embedding space onto two dimensions using UMAP allows for the visualization of the complex structure of scientific mobility at high resolution and at multiple scales, which has implications in practice. 
...
Also, our data can possess survivorship bias. Researchers at prestige organization tend to have more publications, inducing appear more in the mobility sentence.
...
Let $S^+ = \{v_1^+, v_2^+ \cdots v_n^+\}$ and $S^- =\{v_1^-, v_2^- \cdots v_n^-\}$ be the sets of positive and negative poles organization vector respectively. Then, the average vectors of each set can be calculated as $V^{+} = \frac{1}{n}\sum^n_1 v_i^+$ and $V^{-} = \frac{1}{n}\sum^n_1 v_i^-$. From these average vectors of each set of poles, the semantic axis is defined as $V_{\text{axis}} = V^{+} - V^{-}$. Then,  a score of organization $w$ is calculated calculated as the cosine similarity of the organization's vector with the axis, 

$$
score(w)_{V_{\text{axis}}} = \cos(u_{w}, V_{\text{axis}}) = \frac{u_{w} \cdot V_{\text{axis}}}{\lVert v_{i} \rVert \lVert V_{\text{axis}} \rVert},
$$

where higher scores means that organization $w$ is more closely aligned to $S^+$ than $S^-$.
