...
\newtcolorbox[list inside=toc]{sectionbox}[1]{colback=white,
 				       												 colframe=black,
...
While reflecting on these comments and our manuscript, we have made a theoretical breakthrough that addresses the fundamental issues;
...
Namely, we demonstrate that \textit{word2vec}'s objective function is equivalent to the gravity model and thus \textit{word2vec} learns gravity-like relationships between entities, whether it be language or locations.
This development constitutes a major breakthrough in our understanding of neural embeddings and provides a theoretical justification for their application to mobility.

For the second point, we have conducted experiments comparing the performance of neural embedding to (\romannum{1}) a singular value decomposition of the mobility network,  (\romannum{2}) a Laplacian eigenmap embedding  (\romannum{3}) a \textit{word2vec} model with matrix factorization,  and  (\romannum{4}) a direct optimization of the gravity model. We find that the embedding distance from \textit{word2vec} outperforms all baselines, even the direct optimization of gravity model with the multidimensional scaling, in explaining and predicting real-world mobility.

Finally, addressing the third point, we have incorporated relevant literature to better ground our manuscript in the existing studies.
...
\tableofcontents
\clearpage
\todo{@dakoda, could remove comment from the toc?}
...
In addition, this motivates the use of SGNS for the future study of human mobility.
...
This bias in negative sampling turns out to be the key piece that connects word2vec and the gravity model.
...
Specifically, we construct a gravity matrix of $N$ organizations, where each cell of the matrix is calculated as $T_{ij}/m_i m_j$, where $T_{ij}$ is the co-occurrence of organizations $i$ and $j$ in the same affiliation trajectory, and $m_i$ and $m_j$ are the organizations' populations, defined here as the mean annual member of unique mobile and non-mobile authors affiliated. We then embed the gravity matrix using two approaches: a truncated singular-vector decomposition (SVD), and multidimensional scaling (MDS), which embeds each location in an N-dimensional space such that pairwise distances are preserved as well as possible.
...
\response{Thank you for pointing out the missing references!
	...
	Although there are existing works on trajectory-based mobility embedding, we argue that our new result establishing the mathematical equivalence of \textit{word2vec} and the gravity model, as well as our extensive evaluation with many baselines represents a novel and substantial contribution.
}
...
\response{Thank you for this comment. While robustness is a general concern for these methods, we found that their application to our datasets were in fact quite robust, and observed no evidence that findings were the result of methodological artifacts. This is confirmed in extra work done in response to these reviews.
	...
	However, this may not hold true for all problem domains—for example, the natural language includes many millions of tokens, many of which will occur only a few times, allowing stochasticity to have a much stronger effect on the final representation.
...
\response{Thank you for pointing this out.
	We carefully read out the Barthélemy, Physics Reports 499, 1, 2011, and referenced it in our manuscript.
		It has been argued that the power-law decay function fits well with the geographical distance because the power-law gravity model is more suitable for analyzing large, complex, and scale-free spatial systems with long-range spatial memory, whereas the exponential gravity model fits well with the simpler system with no spatial memory~\autocite{chen2015distance}.
	We report the performance of the flux estimation and prediction using different deterrence functions (see Section 2, above), finding that although the absolute values vary, the pattern doesn't change---the embedding distance robustly outperforms geographic distance regardless of the function used. Also, we show why embedding distance fits well with the exponential gravity model with the underlying connection between \textit{word2vec} and the gravity model (see Section 1, above).
...
\response{Thank you for this insightful comment!
	...
	We believe that this new discovery, now included in the manuscript, strongly ground our use of \textit{word2vec} here.

	We agree that the revolution in neural embeddings offers great promise in mobility and look forward to reading and also working on these future studies; however, we argue that our contribution is most well-founded, and best serves the scientific community, by working with simpler and more established models (see Section 2, this letter), especially concerning the mathematical link we uncovered. Moreover, these deep models require a tremendous amount of training data ($\sim$3 billion words in Bert) and model parameters, making their use infeasible in applicants most.

	To highlight the importance of alternative models, however, we add the following line to our conclusion: \say{\textit{Future studies may also consider bi-directional embeddings, such as BERT~\autocite{devlin2018bert}, to incorporate directionality.}}.
...
\response{
	...
	However, given the importance of mobility to science, the importance of human mobility more generally, and the connection we discovered between \textit{word2vec} and the gravity model, that a focus on co-affiliation, which most closely aligns with ideas of mobility, is the most reasonable data to use for this manuscript.
}
...
\response{Thank you for this suggestion.
	We looked into this, using UMAP to reduce the country-level embeddings from 300 down to 30 dimensions, however, the results from this were incomprehensible, much inferior to those using 300 dimensions.
...
\response{Thank you for this idea!
	...
	This is certainly a topic that is in our minds for future study.
