...
\begin{align}
	\label{eq:sigmoid2}
	p(positive|j,i) = \frac{1}{1 + p_0(x)f(x)^{-1}}.
\end{align}

Define $\tilde f(x) = f(x) p_0(x)$ and rewriting the sigmoid function for the negative sampling (Eq.~\ref{eq:sigmoid}) in terms of bias-corrected sigmoid function yields
\begin{align}
	\label{eq:sigmoid3}
	\frac{1}{1 + p_0(x) \tilde f(x)^{-1}}
\end{align}
...
Furthermore, we computed an embedding that attempts to directly-optimize for a gravity-law relationship between relations, one created using singular-vector decomposition (SVD) and another with multidimensional scaling (MDS). 
...
Directly-optimizing the gravity model results in comparable performance to Levy's factorization approach, narroly beating out both in prediction error for certain definitions of organization population. 
Still, the embedding distance, with its stochastic optimization algortihm reliably outperforms these directly-optimized  baselines. 
