...
For the second point, we have also conducted experiments comparing the performance of neural embedding to (\romannum{1}) a singular value decomposition of the mobility network,  (\romannum{2}) a Laplacian eigenmap embedding  (\romannum{3}) a \textit{word2vec} model with matrix factorization,  and  (\romannum{4}) a direct optimization according to the gravity law. We find that the embedding distance from \textit{word2vec} outperforms all baselines, even the direct optimization of gravity law with the multidimensional scaling, in explaining and predicting real-world mobility.
...
%
\begin{equation}
	{\cal J}^{\text{SGNS}} = \frac{1}{T}\sum_{t = 1}^{T} \sum_{-w \leq \tau \leq w, \tau \neq 0} \log P(a_{t + \tau} \vert a_{t}), \label{eq:word2vec-obj}
\end{equation}
where,
...
Although negative sampling is the most common approximation, it is a biased estimator~\autocite{Chia2010,Dyer2014} that does not ensure to yield the optimal embeddings in terms of the objective function, ${\cal J}^{\text{SGNS}}$.
This bias in negative sampling is the indispensable piece between wor2vec and the gravity model.
...
In word2vec, the noise distribution is given by $p_0(\ell) \propto P^\gamma (\ell)$, where $P(j)$ is the fraction of $j$ in the data, and $\gamma>0$ is a hyper-parameter.
Then, for the sampled words, we fit a logistic regression model:
\begin{align}
	\label{eq:logistic-regress}
	P(Y_{j} = 1; \vect{v}_{i}, \vect{u}_j) = \frac{1}{1 + \exp(-\vect{u}_j \cdot \vect{v}_{i})},
\end{align}
by maximizing the log-likelihood:
\begin{align}
	\label{eq:log-likelihood-logistic-regress}
	{\cal J}^{\text{NG}} _j = \sum_{j \in {\cal D}} \left[ Y_{j} \log P(Y_{j} = 1; \vect{v}_{i}, \vect{u}_j) + (1-Y_{j}) \log P(Y_{j} = 0; \vect{v}_{i}, \vect{u}_j)\right],
\end{align}
where ${\cal D}$ is the set of all sampled words.

An underappreciated fact is that maximizing ${\cal J}^{\text{NG}}$ does not guarantee
to yield the optimum embedding in terms of the original word2vec objective function ${\cal J}^{\text{SGNS}}$ even if we increase the training samples and iterations~\autocite{Chia2010,Dyer2014}.
To make this bias explicit, let us consider the unbiased variant of negative samplig, \ie the noise contrastive estimation (NCE)~\autocite{Chia2010,Dyer2014}.
NCE is an unbiased estimator for a probability model $P_m$ of the form:
\begin{align}
	\label{eq:nce-model}
	P_m(x) = \frac{ f(x) }{\sum_{ x' \in {\cal X}} f(x') },
\end{align}
where $f$ is the non-negative likelihood function of data $x$, and ${\cal X}$ is the set of all data.
This general form includes the word2vec model (Eq.~\eqref{eq:cond_prob_w2v}), where $f(x) = \exp(x)$ and $x = \vect{u}_j \cdot \vect{v}_{i}$.
NCE trains the probability model using a binary classification task in the same way as in negative sampling but using a Bayesian formalism for logistic regression.
...
\begin{align}
	\label{eq:prior}
	P_0(Y_{j} = 1) = \frac{1}{k + 1},\quad P_0(Y_{j} = 0) = \frac{k}{k + 1},
\end{align}
...
Assuming that the given data is generated from the probability model to be fit, the class-conditional probability, $P\left(j \vert Y_{j}\right)$, is given by
\begin{align}
	\label{eq:class-cond}
...
\begin{align}
	\label{eq:nce}
	P\left(Y_{j} = 1 \vert j\right) = \frac{
...
\begin{align}
	\label{eq:log-likelihood-logistic-regress}
	{\cal J}^{\text{NCE}} _j = \sum_{j \in {\cal D}} \left[ Y_{j} \log P(Y_{j} = 1\vert j) + (1-Y_{j}) \log P(Y_{j} = 0 \vert j)\right].
\end{align}
by calculating the gradients for embedding vectors $\vect{u}_j$, $\vect{v}_{i}$ and iteratively updating them.
An important consequence of this framework is that NCE is an unbiased estimator that has convergence to the optimal embedding in terms of the original word2vec's objective function, ${\cal J}^{\text{SGNS}}$~\autocite{Chia2010,Dyer2014} if we increase the number of words to sample and the training iterations.
...
\begin{align}
	\label{eq:unbiasef}
	P_m(\vect{u}_j \cdot \vect{v}_{i}) & = \frac{ f(\vect{u}_j \cdot \vect{v}_{i}) }{\sum_{ j' \in {\cal A}} f(\vect{u}_{j'} \cdot \vect{v}_{i}) }, \\
	                                   & = \frac{  p_0(j)\exp( \vect{u}_j \cdot \vect{v}_{i} ) }{Z' _i},
\end{align}
where $Z'_i=\sum_{j' \in \mathcal{A}}  p_0(j')^\gamma \exp(\vect{u}_{j'} \cdot \vect{v}_{i})$.
...
\begin{align}
	\hat{T}_{ij}= N(i) P(j\given i) = \frac{N(i) N(j)^\gamma \exp(\vect{u}_j \cdot \vect{v}_{i})}{Z'_i}. \label{eq:prob_w2v_ng0}
\end{align}
where we have redefined $Z'_i=\sum_{j' \in \mathcal{A}}  N(j)^\gamma \exp(\vect{u}_{j'} \cdot \vect{v}_{i})$.
...
\begin{align}
	\hat{T_{ij}}= N(i) P(j\given i) = \frac{N(i)N(j)\exp(\vect{v}_j \cdot \vect{v}_{i})}{Z'_i}. \label{eq:flow_w2v_ng1}
\end{align}
...
\begin{align}
	\hat{T_{ij}}=  C N(i) N(j)  \exp(\vect{v}_j \cdot \vect{v}_{i}), \label{eq:flow_w2v_ng0}
\end{align}
where $C$ is a positive constant.

In other words, \emph{word2vec with the negative sampling is a equivalent to a gravity model}, with the mass given by the location's frequency $f(i)$, and the distance measured by their dot similarities.
...
\response{Thank you for pointing this out.
	...
	Also, we report the performance of the flux estimation and prediction using different deterrence functions (see Section 2, above), finding that the absolute values vary, the pattern doesn't change---the embedding distance robustly outperforms geographic distance regardless of the function used.
	...
	We find the article that geographical distance ﬁts well with the power-law decay function because power-based gravity model can be more suitable for analyzing large, complex, and scale-free spatial systems with long-range spatial memory, while exponential-based gravity model
	fits well with the simple system with no spatial memory~\autocite{chen2015distance}.
	However, the embedding distance fits well with the exponential decay function, perhaps demonstrating that the real-world spatial complexity of mobility is encoded in the \textit{word2vec} embedding space as simpler pairwise distances, likely resulting from underlying connection we established between \textit{word2vec} and the gravity model (see section 1, above).
...
\response{
	...
	We note that in previous work from our group, we explored the optimal dimensions for graph embedding~\autocite{gu2020dimensions}, finding that while a minimum number of dimensions is necessary, increasing the dimensions does not detract from performance, a property common across deep learning methods.
