...
Here, we answer a following question:å Why does the skip-gram model with negative sampling (SGNS), otherwise known as \textit{word2vec}, work so well to model human mobility?
...
Second, while dot similarity is a formal distance measure in terms of its connection to the gravity model, our simulations show that dot similarity did not predict mobility flow better than cosine similarity (Figs.~\ref{fig:supp:distancemetrics} and \ref{fig:supp:distancemse}). It has been widely observed that cosine similarity is generally better than dot similarity in many applications of word embedding~\cite{} [Jisung, could you add citations?]. Therefore, the superiority of cosine similarity is not a problem specific but general feature of word embedding.
Nevertheless, dot similarity is the second best predictor among the 7 embedding distances. What makes cosine similarity superior to dot similarity warrants the future work.
...
\response{Thank you for pointing this out.
	...
	Also, we report the performance of the flux estimation (Fig.~\ref{fig:supp:distancemetrics}, this document) and prediction (Fig.~\ref{fig:supp:distancermse}, this document) using different deterrence functions, founding that while performance differs, that the embedding distance outperforms geographic
