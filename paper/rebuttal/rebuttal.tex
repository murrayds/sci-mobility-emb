...
Namely, we demonstrate that \textit{word2vec} 's objective function is same as the gravity law and thus \textit{word2vec} learns gravity-like relationships between entities, whether it be language or locations.
...
\label{sec:theory}
Here, we answer a question: Why does the skip-gram model with negative sampling (SGNS), otherwise known as \textit{word2vec}, work so well to model human mobility?
...
First, we outline  \textit{word2vec} model.
...
We used the standard skip-gram negative sampling (SGNS) word embedding, commonly known as \textit{word2vec}~\autocite{mikolov2013word2vec}.
...
\subsection{Gap between the mathematical analysis and numerical simulations}

In the mathematical analysis, we assumed that the embedding vectors are optimal in terms of the objective of {\it word2vec}, which, however, is not true in our numerical simulations.
This violation of assumption leads to two substantial consequences for embedding vectors.
First, although there exist optimal in-vector and out-vector that are identical, the in-vector and out-vector can be different in practice because they are sub-optimal.
Second, while dot similarity is a formal distance measure in terms of its connection to the gravity model, our simulations show that dot similarity did not predict mobility flow better than cosine similarity (Fig.~\ref{fig:supp:distancemetrics}). It has been widely observed that cosine similarity is generally better than dot similarity in many applications of word embedding~\autocite{levy2015improving}. Therefore, the superiority of cosine similarity is not a problem specific to, but instead a general feature of word embedding.
Nevertheless, dot similarity still outperformed geographic distance and some baselines (Tables~\ref{supp:table:r2_table},~\ref{supp:table:rmse_exp_table},~\ref{supp:table:rmse_power_table}). What makes cosine similarity superior to dot similarity warrants the future work.

\subsection{Changes to the manuscript}
To reflect these breakthroughs, we have made the following changes to the manuscript:
\begin{itemize}
	\itemsep0em
	...
	\item Since the formulation of the gravity used in the above derivations assumes $\gamma = 1$, we use this value of $gamma$ for all reported results. No result substantively changes with this change.
	\item \todo{We added a sentence to the paper speculating on why cosine similarity works best}
	\item \todo{We also added a sentence noting the in-vector and out-vector issue, and speculating on reasons/consequences}
\end{itemize}
...
In addition to the new baselines, we have also incorporate additional experimental parameters into our analysis.
...
The default of the `\texttt{gensim} implementation of \textit{word2vec} is to set $\gamma = 0.75$, and this was the value used in our original manuscript.
...
This performance held when considering both the exponential and the power-law forms of the gravity model of mobility. 
The euclidean distance between directly-factorized vectors performs comparably, and even beats out the embedding distance under certain experimental conditions (Tables~\ref{supp:table:r2_table},~\ref{supp:table:rmse_exp_table},~\ref{supp:table:rmse_power_table}). 


\subsection{Why do neural embeddings outperform direct matrix factorization?}% why sgd is better?
One question raised by these comparisons is, why do neural-embedding approaches, which rely on stochastic gradient descent, outperform direct matrix factorization, especially seeing as \textit{word2vec} implicitly approximates factorization?
...
\begin{itemize}
	...
	\item \todo{add sentence speculating on SGD performance differences into text/supporting meterials}
\end{itemize}
...
They have now been incorperated into our manuscript's introduction.
}
...
\response{
	...
	Since our original submission, we have made a breakthrough, uncovering a mathematical connection between the skip-gram model with negative sampling, and the gravity model of mobility.
	Namely, we find that SGNS is a gravity model, and is learns gravity-like relationships between tokens in its vocabulary. 
	...
	We find that in nearly every case, SGNS outperforms all of these baselines, furhter illustrating the potential of such models for embedding and studying mobility. 
...
\response{Thank you for pointing this out.
	...
	We also argue that geographical distance ﬁts well with the power-law decay function because power-based gravity model can be more suitable for analyzing large, complex, and scale-free spatial systems and the exponential-based gravity model indicates simplicty rather than spatial complexity~\autocite{chen2015distance}. Meanwhile, embedding distance ﬁts well with the exponential decay function, which we demonstrate to correspond to an underlying mathematical relationship between \textit{word2vec} and the gravity model, a connection we recently discovered and included as a major finding in this revision (see our response to Referee 1).
...
\response{
\todo{Thoughts on this response? Still not 100\% sure I follow the reviewer}
