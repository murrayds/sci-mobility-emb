...
$A$ is the co-occurrence matrix of $C$ organizations with element $A_{ij}$ indicating the number of co-occurrences of organizations $i$ and $j$ in the same affiliation trajectory, and $D$ is the diagonal matrix with diagonals $D_{ii}=\sum_j {A_{ij}}$. We set the dimension to $d=300$.
...
First, we construct the $C\times C$ matrix $M$ whose entries $M_{ij}$ are given by
\begin{equation}
