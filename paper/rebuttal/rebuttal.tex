...
A target-context word pair $(i,j)$ is sampled from the given data as a positive pair, and $k$ negative pairs $(i,\ell)$ are sampled from a noise distribution $p_0(\ell)=N(\ell)^\gamma / \sum_{x}{N(x)^\gamma} $. ($N(\ell)$ is frequency of $\ell$ in the dataset, $\gamma>0$ is a parameter).
...
So, the model with the negative sampling optimizes $\tilde f(x)$, not $f(x)$. In case of the {\it word2vec}, the instance is the dot similarity $x=\vect{u}_i \cdot \vect{v}_j$,  $f(x)=\exp(x)$, and $p_0(x)=p_0(j)=N(j)^\gamma / \sum_{x}{N(j)^\gamma}$, and the SGNS word2vec actually optimize the following conditional probability:
\begin{equation}
\begin{split}
	P\left(j \given i \right) &:= \frac{N(j)^\gamma \exp(\vect{u}_j \cdot \vect{v}_{i}) / \sum_{x}{N(x)^\gamma}}{\sum_{j'\ \in \mathcal{A}} N(j')^\gamma  \exp(\vect{u}_j' \cdot \vect{v}_{i}) /\sum_{x}{N(x)^\gamma} } \\
	&  = \frac{N(j)^\gamma \exp(\vect{u}_j \cdot \vect{v}_{i})}{\sum_{j'\ \in \mathcal{A}} N(j')^\gamma \exp(\vect{u}_{j'} \cdot \vect{v}_{i})} ,\\
	& = \frac{N(j)^\gamma \exp(\vect{u}_j \cdot \vect{v}_{i})}{Z'_i} ,
	\end{split}
	\label{eq:real_cond}
\end{equation}
where $Z'_i=\sum_{j' \in \mathcal{A}}  N(j')^\gamma \exp(\vect{u}_{j'} \cdot \vect{v}_{i})$.
