...
In fact, dot similarity calculated by {\it word2vec} is the second-best predictor of mobility flow among ten embedding distances in our numerical simulations~(Tables~\ref{supp:table:r2_table},~\ref{supp:table:rmse_exp_table},~\ref{supp:table:rmse_power_table}).
...
Therefore, the inferiority of dot similarity is attributed to the optimization process, and therefore, dot similarity can outperform cosine similarity by a careful tuning of the optimization algorithm.
%Nevertheless, among the ten embedding distance measures tested in our simulations, dot similarity is the second-best predictor of mobility flow, can be even better than cosine similarity by carefully turning the optimization algorithm.
