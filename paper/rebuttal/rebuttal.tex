...
\begin{align}
	...
	{\cal J}^{\text{NS}} _j = \sum_{j \in {\cal D}} \left[ Y_{j} \log P(Y_{j} = 1; \vect{v}_{i}, \vect{u}_j) + (1-Y_{j}) \log P(Y_{j} = 0; \vect{v}_{i}, \vect{u}_j)\right],
\end{align}
...
\begin{align}
	\label{eq:nce}
	P^{NCE}\left(Y_{j} = 1 \vert j\right) = \frac{
...
\begin{align}
	\label{eq:log-likelihood-logistic-regress}
	{\cal J}^{\text{NCE}} _j = \sum_{j \in {\cal D}} \left[ Y_{j} \log P^{NCE}(Y_{j} = 1\vert j) + (1-Y_{j}) \log P^{NCE}(Y_{j} = 0 \vert j)\right].
\end{align}
...
\end{align}
where  $P(j)$ is the fraction of $j$ in the data. Taking together, conditional probability of SGNS \textit{word2vec} actually optimize is
\begin{align}
...
\begin{align}
	\hat{T}_{ij} \propto  P(i) P(j\given i) \propto  \frac{P(i) P(j)^\gamma \exp(\vect{u}_j \cdot \vect{v}_{i})}{Z'_i}. \label{eq:prob_w2v_ng0}
\end{align}
