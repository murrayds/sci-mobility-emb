...
To make this bias more explicit, let us consider an unbiased estimator similar to negative sampling, \ie the noise contrastive estimation (NCE).

NCE is an unbiased estimator for probability models of the form:
\begin{align}
	\label{eq:prior}
	\frac{ \exp\left( f(x) \right) }{ \sum_{ x' \in {\cal X}} \exp\left( f(x') \right) }
\end{align}


NCE trains a model using a binary classification task in the same way as in negative sampling but using Bayesian formalism for logistic regression.
Specifically, before the training, we have a prior belief that $1$ in $1+k$ words is sampled from the given data, which can be modeled as prior probabilities
\begin{align}
	\label{eq:prior}
...
\end{align}
Bearing in mind that word $j$ is sampled from the given data if $Y_{j}=1$ and from the noise distribution $p_0$ if $Y_j = 0$. Therefore, the class-conditional probability, $P\left(j \vert Y_{j}\right)$, is given by
\begin{align}
	\label{eq:class-cond}
	P(j \vert Y_{j} = 1) & = \frac{\exp(\vect{u}_j \cdot \vect{v}_{i})}{Z_i},
	\quad
...
\begin{align}
	...
	P\left(Y_{j} = 1 \vert j\right) = \frac{
		\exp(\vect{u}_j \cdot \vect{v}_{i})/Z_i
	}{
		\exp(\vect{u}_j \cdot \vect{v}_{i})/Z_i  + kp_0(j)
	},
\end{align}
...
\begin{align}
	...
	P\left(Y_{j}=1 \vert j\right) = \frac{
		...
		1 + \exp\left[ - \vect{u}_j \cdot \vect{v}_{i}  + \ln p_0(j) - c \right]
	},
\end{align}
where $c = \ln Z_i - \ln k$ is a constant.
NCE uses the posterior probability to predict the label of individual word.
The log-likelihood for the posterior probability is given by
\begin{align}
	\label{eq:log-likelihood-logistic-regress}
	{\cal J}^{\text{NCE}} _j = \sum_{j \in {\cal D}} \left[ Y_{j} \log P(Y_{j} = 1\vert j) + (1-Y_{j}) \log P(Y_{j} = 0 \vert j)\right].
\end{align}
Crucially, NCE is an unbiased estimator that ensures the convergence to the optimal embedding in terms of the original word2vec's objective function, ${\cal J}^{\text{SGNS}}$~\autocite{Chia2010,Dyer2014}.

Returning to negative sampling, what model does negative sampling train?
The logistic regression model of negative sampling (Eq.~\eqref{eq:logistic-regress}) can be rewritten in form of that of NCE as follow:
\begin{align}
	P\left(Y_{j}=1 \vert j\right) = \frac{
		1
	}{
