...
\response{Thank you for pointing this out.
	We carefully read out the Barthélemy, Physics Reports 499, 1, 2011, and referenced it in our manuscript.
		It has been argued that the power-law decay function fits well with the geographical distance because power-law gravity model is more suitable for analyzing large, complex, and scale-free spatial systems with long-range spatial memory, whereas exponential gravity model
	fits well with the simpler system with no spatial memory~\autocite{chen2015distance}.
	We report the performance of the flux estimation and prediction using different deterrence functions (see Section 2, above), finding that although the absolute values vary, the pattern doesn't change---the embedding distance robustly outperforms geographic distance regardless of the function used. Also, we shows why embedding distance fits well with exponential gravity model with the underling connection between \textit{word2vec} and the gravity model (see Section 1, above).
...
\response{Thank you for this idea!
	...
	Then, using that distance, one could measure the ``embedding distance'' spanned by all authors in the byline, and answer the question whether typical collaborations relate to impact or not.
