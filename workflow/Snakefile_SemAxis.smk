...
rule generate_prestige_axis_org_codes:
    ...
    run:
        if "times" in {wildcards.ranking}:
            shell(
                "Rscript scripts/GeneratePrestigeAxisOrgCodes.R --input {input} --output {output} \
...
rule generate_prestige_axis_org_codes_other_countries:
    input:
        ORG_RANKINGS,
    output:
    ...
    params:
        lookup=rules.add_state_to_lookup.output,
    shell:
        "Rscript scripts/GeneratePrestigeAxisOrgCodes_OtherCountries.R --input {input} --output {output} \
        --variable impact_frac_mncs --lookup {params.lookup} \
        --norgs {wildcards.numorgs} --country {wildcards.country}"


rule generate_usa_coasts_axis_org_codes:
    input:
        rules.add_state_to_lookup.output,
    output:
        COASTS_AXIS_ORGS,
    shell:
...
rule calculate_semaxis_prestige_projections:
    ...
    output:
        PRESTIGE_AXIS_PROJECTIONS,
    shell:
        "python scripts/calculate_SemAxis_projections.py --input {input.w2v} \
        --axis {input.axis} --output {output}"


rule calculate_semaxis_prestige_projections_other_countries:
    input:
        w2v=rules.train_word2vec_model.output,
        axis=rules.generate_prestige_axis_org_codes_other_countries.output,
    output:
...
rule generate_aggregate_prestige_rank_correlations:
    input:
        axes=[
            expand(
                rules.calculate_semaxis_prestige_projections.output,
                traj=TRAJECTORIES,
