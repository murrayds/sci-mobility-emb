#
# PlotGravityLawRelationshipAirport.R
#
...
option_list = list(
  make_option(c("-i", "--input"), action="store", default=NA, type='character',
              help="Path to file containing aggregate airport distances"),
...
dist <- dist %>%
  select(geo_distance, emb_distance, gravity)
...
plot <- dist %>%
  ggplot(aes(x = distance, y = gravity_logged)) +
    ...
    coord_fixed() +
    guides(fill = F) +
    scale_y_continuous(breaks = c(-14, -12, -10, -8, -6, -4, -2),
                       labels = function(x) { parse(text = paste0("10^", x)) },
                       limits = c(-12, -1)
    ) +
