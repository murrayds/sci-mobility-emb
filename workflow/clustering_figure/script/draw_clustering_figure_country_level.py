...
def draw_figure(
    ...
    country_to_continent = country_meta_data.set_index('wos_country')['continent'].to_dict()
    country_to_language = country_meta_data.set_index('wos_country')['language'].to_dict()
    country_to_language_family = country_meta_data.set_index('wos_country')['ethnologue_family'].to_dict()
    ...
    target_countries = [k for k,v in  Counter(country_list).items() if v >= 25 and k != 'United States']
    representative_vector = [embedding_list[country_list == country].mean(axis=0) for country in target_countries]
    ...
    language_family_list = [country_to_language_family[country] for country in target_countries]
    ...
    continent_color_mapper = {"South America": 5,
                            "Africa": 8,
                            "Europe": 2,
                            "Asia": 3,
                            "Oceania": 4,
                            "North America": 0}
    languae_color_mapper ={'en': '#D6A023',
                       'zh': '#FF5722',
                       'de': '#F5CC26',
                       'pt': '#009688',
                       'nl': '#BDA661',
                       ...
                       'ar': "#d3d3d3",
                       'af': "#d3d3d3"}
    languae_family_color_mapper = {'Italic': c_list[10],
                               'Germanic': c_list[5],
                               'Sintic': c_list[3],
                               'Slavic': c_list[12],
                               'Semitic': c_list[0],
    ...
    languae_family_color_list = [languae_family_color_mapper[i] for i in language_family_list]
    ...
    leaves_color_dict = {name: c_list[leaves_color_mapper[cluster_index]] for name, cluster_index in zip(target_countries, cluster_result)}
    ...
    # Draw heatmap part
    g = sns.clustermap(sim_mtx,
                       cmap='viridis_r',
                       yticklabels=target_countries,
                       vmax= 0.776,
                       row_linkage=linked,
                       col_linkage=linked,
                       row_colors=[continent_color_list , languae_color_list, languae_family_color_list],
                       linewidths=.01,
                      figsize=(20,16),
    ...
    for label in ax.get_yticklabels():
        ax.text(label._x - 10, label._y + 0.22, label.get_text(), c = leaves_color_dict [label.get_text()], fontproperties=small_prop)
    ax.set_yticks([])
    ...
    ax.add_patch(Rectangle((-4.6, 0), 4.6, 7, fill=False, clip_on=False, edgecolor='black', lw=3))
    ax.add_patch(Rectangle((-4.6, 7), 4.6, 4, fill=False, clip_on=False, edgecolor='black', lw=3))
    ax.add_patch(Rectangle((-4.6, 11), 4.6, 6, fill=False, clip_on=False, edgecolor='black', lw=3))
    ax.add_patch(Rectangle((-4.6, 17), 4.6, 8, fill=False, clip_on=False, edgecolor='black', lw=3))
    ax.add_patch(Rectangle((-4.6, 25), 4.6, 6, fill=False, clip_on=False, edgecolor='black', lw=3))
    ax.add_patch(Rectangle((-4.6, 31), 4.6, 5, fill=False, clip_on=False, edgecolor='black', lw=3))
    ...
    similiarties_continent = [sim.element_sim(hierarchical_clu, continent_clu, r=r) for r in r_list]
    similiarties_language = [sim.element_sim(hierarchical_clu, language_clu, r=r) for r in r_list]
    similiarties_language_family = [sim.element_sim(hierarchical_clu, language_family_clu, r=r) for r in r_list]
